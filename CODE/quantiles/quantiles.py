...
length_of_text = length_of_text.groupby(['asin'])['average_length_of_review_text'].mean().to_frame().reset_index()
...
final = final.sort_values(['overall_average', 'overall_count','overall_stdev','average_length_of_review_text'], ascending=[False, False, True, False])
...
conn.close()
print("Quantile calculation complete.  Elapsed time: {}".format(datetime.now()- start_time))
